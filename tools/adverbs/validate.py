#!/usr/bin/env python3
"""Check one adverb JSON file against tools/adverbs/FORMAT.md.

    python3 tools/adverbs/validate.py tools/adverbs/groups/06-time.json

Writers run this on their own file before finishing, so the classes of error that
cost rework in the noun and verb passes — a tag label that is really prose, a bare
/e/, an unbalanced asterisk, an entry whose examples never show its comparative —
are caught by the person who can fix them, not by the build.
"""
import json, re, sys

TYPES = {'manner', 'place', 'direction', 'time', 'duration', 'frequency', 'degree', 'focusing',
         'viewpoint', 'stance', 'linking', 'interrogative', 'relative', 'negative', 'particle'}
POSITIONS = {'front', 'mid', 'end', 'front or end', 'mid or end', 'any position',
             'before its word', 'after its word'}
COMPARISONS = {'not gradable', 'more/most', '-er/-est', 'irregular'}
    ...
    for i, e in enumerate(entries):
        ...
        for n, s in enumerate(senses, 1):
            ...
            for x in ex:
                ...
                sp = spans(x)
                all_spans += sp
                hit = any(words & set(re.findall(r"[a-z']+", s_)) or s_ in forms for s_ in sp)
                if not hit:
                    problems.append(f'{where}: headword not asterisked in: {x[:60]}')
        if comp in ('-er/-est', 'irregular'):
            if not forms:
                problems.append(f'{where}: comparison "{comp}" must list the forms, e.g. -er/-est — *faster*, *fastest*')
            elif not any(s_ in forms for s_ in all_spans):
